...
- Región: `us-east-1`
- Plan: gratuito ($0/mes)
- 38 migraciones aplicadas al remoto (`list_migrations`, 2026-09-07): las
...
|---|---|---|---|
| `registrar_marca_kiosko(pin, kiosko_id)` | `fn_registrar_marca_kiosko` | `anon`, `authenticated` **a propósito** (el kiosko no tiene sesión) — función interna sin `EXECUTE` para nadie más que su dueño desde `20260905000005` | **Reescrita en F1.3 (2026-09-07)**: valida formato de PIN y bcrypt contra `rrhh.contrato_credenciales` de contratos `activo` de la empresa del kiosko (ya NO contra `rrhh.empleados.pin_hash/estado` — deprecados desde F1.1), nunca recibe `company_id` del cliente (lo deriva del `kiosko_id`), aplica el rate-limit persistente de `rrhh.kiosko_rate_limits` (8 fallos/1min → bloqueo 5min, el bloqueo siempre prevalece sobre el reinicio de ventana), alterna entrada/salida, inserta en `asistencia_marcas`. Rechaza con `RETURN` (nunca `RAISE EXCEPTION`). No devuelve datos personales del empleado. Verificado end-to-end (activar→marcar→regenerar→PIN viejo rechazado→finalizar→PIN rechazado) |
